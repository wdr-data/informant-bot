    ...
    if (payload.type === 'push') {
        return request({
            ...
            reportCloudWatch('Informant/Activity', 'PushInteraction', {
                ...
                reportIndex: next_report_index - 1,
                isFirstFragment: payload.action === 'report_start',
                isLastFragment: !button,
            });

            return send_fragment(chat, messages, quick_replies);

        }).catch(error => {
            console.log('Error loading push:', error);
            return chat.sendText('Das kann ich leider nicht finden 🙁 Und dabei habe ich wirklich überall gesucht 🕵');
        })
    } else {
        return send_fragment(chat, messages, quick_replies).then(() => {
    ...
    return new Promise((resolve, reject) => {
        ...
        const sendNext = () => {
            ...
            if (message.type === TEXT && messages.length > 1) {
                ...
                return chat.sendText(message.content, quick_replies).then(() => {
                    return sendNext();
                }).catch((error) => {
