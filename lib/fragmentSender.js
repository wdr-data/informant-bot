    ...
    if (button) {
        buttonNext = quickReply(
            button.question, {
                ...
                event: payload.event,
                label: isNaN(payload.label) ? 1 : Number(payload.label) + 1,
            });
                    ...
                    quickReplies.push(
                        quickReply(
                            'Nächstes Thema',
                            {
                                ...
                                preview: payload.preview,
                                category: payload.category,
                                event: push.reports[nextReportIndex].headline,
                                label: 'intro',
                            })
                    );
                } else {
                    quickReplies.push(
                        quickReply(
                            'Reicht jetzt',
                            {
                                ...
                                category: payload.category,
                                event: 'outro',
                                label: payload.before ? `${payload.before.length}-reports` :
                                    'classic',
                            })
                    );
                }
                ...
                quickReplies.push(
                    quickReply(
                        'Reicht jetzt',
                        {
                            ...
                            category: payload.category,
                            event: 'outro',
                            label: payload.before ? `${payload.before.length}-reports` : 'classic',
                        })
                );
            }
                    ...
                    quizButtons.push(
                        buttonPostback(
                            option.quiz_option,
                            {
                                ...
                                category: payload.category,
                                event: payload.event,
                                label: `${option.correct_option}-${option.quiz_option}`,
                            }));
                }
            } else if (!buttonNext && payload.link) {
                buttonLink = buttonUrl('Mehr 🌍', payload.link);
...
const makeQuizButtons = async (buttons, push, reportIndex, type, readReports, preview) => {
    const params = {
