...
import { quickReply, buttonPostback } from './facebook';
...
export default async (chat, fragments, payload, initialMessage = '', initialMedia = null) => {
    ...
    if (payload.type === 'push') {
        try {
            ...
            const isQuiz = push.reports[currentReportIndex].is_quiz;
            ...
            if (!('before' in payload)) {
                ...
                if (!buttonNext && isQuiz) {
                    makeQuizButtons(quizButtons, push, payload.type);
                }
            } else {
                ...
                } else if (!buttonNext && isQuiz) {
                    makeQuizButtons(quizButtons, push, payload.type, readReports);
                }
                const openReports = [];
            ...
            await sendFragment(chat, messages, quickReplies, quizButtons);
...
const makeQuizButtons = async (buttons, push, type, readReports) => {
    const quiz = await request({
        uri: urls.quizByReport(push.report.id),
        json: true,
    });
    const payload = {
        action: 'quiz_option',
...
const sendFragment = async (chat, messages, quickReplies, quizButtons) => {
    const errors = [];
        ...
        if (message.type === TEXT && messages.length > 1) {
            ...
            try {
                if (quizButtons.length) {
                    await chat.sendButtons(message.content, quizButtons, quickReplies);
                } else {
