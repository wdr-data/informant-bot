...
function getLatestPush(timing) {
    ...
    return request.get({uri: urls.pushes, json: true, qs: {timing: timing, pub_date: isoDate, limit: 1}})
        .catch(e => {
...
function assemblePush(push) {
    const introHeadlines = [push.intro].concat(push.reports.map(r => `➡ ${r.headline}`)).join('\n');
