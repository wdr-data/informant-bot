...
export class Chat {
    ...
    async addLabel(label) {
        ...
        try {
            const result = await rp.post({
                uri: `https://graph.facebook.com/v3.3/${labelId}/label`,
                json: true,
                qs: {
                    'access_token': process.env.FB_PAGETOKEN,
                },
    ...
    async getLabels() {
        const body = await rp.get({
            uri: `https://graph.facebook.com/v3.3/${this.event.sender.id}/custom_labels`,
            qs: {
                fields: 'name',
                'access_token': process.env.FB_PAGETOKEN,
            },
        ...
        const sendBroadcast = async function(labelId) {
            ...
            try {
                ...
                const insights = await rp.get({
                    uri: `https://graph.facebook.com/v3.3/${body.broadcast_id}/insights/messages_sent`,
                    json: true,
                    qs: {
                        'access_token': process.env.FB_PAGETOKEN,
                    },
