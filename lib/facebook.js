...
const { storeUser, updateUser, deleteUser } = require('../fetch_users');
      ...
      rp.post(
        {
          ...
          let item = {
            morning: (label == 'push-morning') ? 1 : 0,
            evening: (label == 'push-evening') ? 1 : 0,
          };
          storeUser(this.event.sender.id, item).then(() => {
            console.log(`Created in dynamoDB ${this.event.sender.id} with ${label}`);
          }).catch(error => {
            console.log('Creating user in dynamoDB failed: ' + error);
            updateUser(
              this.event.sender.id,
              (label == 'push-morning') ? 'morning' : 'evening',
              1).then(() => {
                console.log(`Updated in dynamoDB ${this.event.sender.id} with ${label}`);
              }).catch(error => {
    ...
    getLabelID(label).then(label_id => {
      rp.delete(
        ...
        updateUser(
          this.event.sender.id,
          (label == 'push-morning') ? 'morning' : 'evening',
          0).then(data => { //{ Attributes: { psid: '1652906054773251', evening: 0, morning: 1 } }
            if (data.Attributes.morning == 0 && data.Attributes.evening == 0) {
              deleteUser(this.event.sender.id).then(() => {
                console.log(`Deleted User in dynamoDB with psid ${this.event.sender.id}`);
              }).catch(error => {
                console.log(`Delete user from dynamoDB failed: ${error}`);
              })
            }
          })
      }).catch(error => {
