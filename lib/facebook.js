      ...
      rp.post(
        {
          ...
          const item = {
            morning: (label === 'push-morning'),
            evening: (label === 'push-evening'),
          };

          subscriptions.create(this.event.sender.id, item).then(() => {
            console.log(`Created in dynamoDB ${this.event.sender.id} with ${label}`);
          }).catch(error => {
            console.log('Creating user in dynamoDB failed: ' + error);

            let timing = null;
            if (label === 'push-morning') {
              timing = 'morning';
            } else if (label === 'push-evening') {
              timing = 'evening';
            } else {
            ...
            subscriptions.update(this.event.sender.id, timing, true).then(() => {
  ...
  removeLabel(label) {
    getLabelID(label).then(label_id => {
      rp.delete(
        ...
        subscriptions.update(this.event.sender.id, timing, false).then(sub => {
            //{ Attributes: { psid: '1652906054773251', evening: 0, morning: 1 } }
            if (!sub.morning && !sub.evening) {
              subscriptions.remove(this.event.sender.id).then(() => {
